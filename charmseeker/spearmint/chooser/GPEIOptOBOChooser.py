...
class GPEIOptChooser:
    ...
    def __init__(self, expt_dir, covar="Matern52", mcmc_iters=10,
                 ...
                 grid_subset=5, use_multiprocessing=False, budget=1):
    ...
    def next(self, expt_grid, grid, values, candidates, pending, complete):
        ...
        #     next_grid = np.array([self.budget for i in xrange(grid.shape[1])])
        #     return 0, next_grid
        ...
        if complete.shape[0] < 2:
            return int(candidates[npr.randint(0, len(candidates))])
        ...
        if self.mcmc_iters > 0:
            ...
            if self.needs_burnin:
                for mcmc_iter in xrange(self.burnin):
                    ...
                    log("BURN %d/%d] mean: %.2f  amp: %.2f noise: %.4f  min_ls: %.4f  max_ls: %.4f"
                        % (mcmc_iter+1, self.burnin, self.mean, np.sqrt(self.amp2),
            ...
            overall_ei = self.ei_over_hypers(expt_grid, comp, pend, cand2, vals)
            ...
            overall_ei = self.ei_over_hypers(expt_grid, comp, pend, cand, vals)
    ...
    def ei_over_hypers(self, expt_grid, comp, pend, cand, vals):
