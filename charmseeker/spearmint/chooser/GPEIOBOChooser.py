...
def init(expt_dir, arg_string):
    ...
    return GPEIOBOChooser(expt_dir, **args)

"""
This the chooser module for the outer layer bayesian optimization.
The constraint is a indicator functions to indicate budget vectors that
satisfy the pipeline budget constraint.
"""
class GPEIOBOChooser:

    def __init__(self, expt_dir, covar="Matern52", mcmc_iters=10,
                 pending_samples=100, noiseless=False, budget=1):
    ...
    def next(self, expt_grid, grid, values, candidates, pending, complete):
        ...
        if complete.shape[0] < 1:
            budget = 0.6
            satisfied = candidates[(np.sum(cand, axis=1) <= budget)]
            return satisfied[npr.randint(0, len(satisfied))]
        ...
        if self.mcmc_iters > 0:
            ...
            for mcmc_iter in xrange(self.mcmc_iters):
                ...
                log("mean: %f  amp: %f  noise: %f  min_ls: %f  max_ls: %f" % (self.mean, np.sqrt(self.amp2), self.noise, np.min(self.ls), np.max(self.ls)))
                # TODO: @Miao change
            ...
            # Miao: assume that the probability of hyperparameters is equal.
            ...
            # Miao: another method, use a single group of optimized hyperparameters rather than full bayesian.
            try:
            ...
            ei = self.compute_ei(expt_grid, comp, pend, cand, vals)
    ...
    def compute_ei(self, expt_grid, comp, pend, cand, vals):
        # TODO: @miao get all parameters and sum them up
        # params.shape is (cand.shape(0),)
        params = np.sum(expt_grid.get_all_params(cand), axis=1)

        # TODO: indicator function: probability of constraints satisfied.
        print("Pipeline budget: ", self.budget)
        weight = (params <= self.budget)
        ...
        if pend.shape[0] == 0:
            ...
            # Miao: Gaussian Process Regression
            ...
            # return ei
            log("Weighted EI: ", np.nonzero(ei * weight)[0].shape)
            return ei * weight
        else:
            ...
            # Make predictions.
            # TODO: predict pend_chol.shape = (pend.shape[0],)
            # sample #pending_samples values for each pend sample
            # pend_chol.shape is (pend.shape[0], pend.shape[0])
            ...
            # ei.shape is (cand.shape[0], self.pending_samples)
            log("Weighted EI: ", np.nonzero(weight)[0].shape)
            return np.mean(ei, axis=1) * weight
