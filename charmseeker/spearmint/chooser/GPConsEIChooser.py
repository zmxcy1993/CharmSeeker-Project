...
class GPConstrainedEIChooser:
    ...
    def next(self, grid, values, durations, candidates, pending, complete):
        ...
        # best_comp is the minimum of all complete values no matter good or bad values.
        ...
        if self.mcmc_iters > 0:
            ...
            for mcmc_iter in xrange(self.mcmc_iters):
                ...
                # sampling hyperparameters for objective GP only use good values.
                # Thus Guassian regression only use good values.
                self.sample_hypers(comp[goodvals, :], vals[goodvals])
                ...
                if self.verbosity > 0:
                    ...
                    log("%d/%d] constraint_mean: %.2f constraint_amp: %.2f constraint_gain: %.4f "
                                     "constraint_min_ls: %.4f constraint_max_ls: %.4f "
    ...
    def pred_constraint_voilation(self, cand, comp, vals):
        ...
        func_m = np.dot(cand_cross.T, alpha)
    ...
    def compute_constrained_ei(self, comp, pend, cand, vals, labels):
        # First we make predictions for the durations as that doesn't depend on pending experiments
