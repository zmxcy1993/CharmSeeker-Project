...
def optimize_pt(c, b, expt_grid, comp, pend, vals, model):
    ret = spo.fmin_l_bfgs_b(model.grad_optimize_ei_over_hypers, c.flatten(),
                            args=(expt_grid, comp, pend, vals), bounds=b, disp=0)
    return ret[0]

def init(expt_dir, arg_string):
    args = util.unpack_args(arg_string)
    return GPEIOBOOptChooser(expt_dir, **args)
...
class GPEIOBOOptChooser:
    ...
    def next(self, expt_grid, grid, values, candidates, pending, complete):
        self.iteration += 1
        if expt_grid.get_real_complete() < 2:
            return int(candidates[0])

        print("complete id: ", complete)
        ...
        # get valid best so far and spray a set of cand around this best
        best_values = np.min(vals)
        best_comp = np.nonzero(vals == best_values)[0][0]
        ...
        if self.mcmc_iters > 0:
            ...
            if self.needs_burnin:
                for mcmc_iter in xrange(self.burnin):
                    ...
                    log("BURN %d/%d] mean: %.2f  amp: %.2f noise: %.4f  min_ls: %.4f  max_ls: %.4f"
            ...
            for mcmc_iter in range(self.mcmc_iters):
                self.sample_hypers(comp, vals)
                log("%d/%d] mean: %.2f  amp: %.2f  noise: %.4f min_ls: %.4f  max_ls: %.4f"
                    % (mcmc_iter+1, self.mcmc_iters, self.mean, np.sqrt(self.amp2), self.noise,
                       np.min(self.ls), np.max(self.ls)))
            self.dump_hypers()
            ...
            if self.use_multiprocessing:
                ...
                results = [pool.apply_async(optimize_pt, args=(
                            c, b, expt_grid, comp, pend, vals, copy.copy(self))) for c in cand2]
                for res in results:
                ...
                for i in xrange(0, cand2.shape[0]):
                    ...
                    ret = spo.fmin_l_bfgs_b(self.grad_optimize_ei_over_hypers, cand2[i, :].flatten(),
                                            args=(comp, pend, vals, True), bounds=b, disp=0)
                    cand2[i, :] = ret[0]
            ...
            log("mean: %.2f  amp: %.2f  noise: %.4f  min_ls: %.4f  max_ls: %.4f"
            ...
            for i in xrange(0, cand2.shape[0]):
                ret = spo.fmin_l_bfgs_b(self.grad_optimize_ei,
                                        cand2[i, :].flatten(), args=(comp, vals, True), bounds=b, disp=0)
                cand2[i, :] = ret[0]
            cand = np.vstack((cand, cand2))
    ...
    def grad_optimize_ei(self, cand, comp, pend, vals, compute_grad=True):
        if pend.shape[0] == 0:
            ...
            fant_vals = np.concatenate(
                (np.tile(vals[:, np.newaxis], (1, self.pending_samples)), pend_fant))
    ...
    def compute_ei(self, comp, pend, cand, vals):
        if pend.shape[0] == 0:
            ...
            print("predicted mean: ", func_m)
            print("predicted variance: ", func_v)
            print("EI: ", ei)
